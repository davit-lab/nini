...
function ShotsAdmin() {
  ...
  const { data: shots = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("shots").select("*").order("sort_order");
  ...
  async function add() {
    ...
    const { error } = await supabase.from("shots").insert({
      url: draft.url, caption: draft.caption || null, sort_order: shots.length,
    });
  ...
  async function updateCaption(id: string, caption: string) {
    await supabase.from("shots").update({ caption }).eq("id", id);
    qc.invalidateQueries({ queryKey: ["admin-shots"] });
  }
  ...
  async function moveSort(id: string, dir: -1 | 1) {
    ...
    await Promise.all([
      supabase.from("shots").update({ sort_order: b.sort_order }).eq("id", a.id),
      supabase.from("shots").update({ sort_order: a.sort_order }).eq("id", b.id),
    ]);
    qc.invalidateQueries({ queryKey: ["admin-shots"] });
    qc.invalidateQueries({ queryKey: ["shots"] });
  }
  ...
  return (
    <div>
      <span className="micro-label">Manage</span>
      <h1 className="mt-3 text-5xl font-display font-black uppercase tracking-editorial mb-10">Shots</h1>
      ...
      <div className="mb-10 p-6 rounded-2xl bg-card border border-border">
        <div className="grid md:grid-cols-[200px,1fr,auto] gap-4 items-end">
          ...
          <input className="bg-background border border-border rounded-xl px-4 py-3 outline-none focus:border-primary"
            placeholder="Caption (optional)" value={draft.caption}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {shots.map((s, i) => (
          <div key={s.id} className="rounded-xl overflow-hidden bg-card border border-border group">
            ...
            <input
              defaultValue={s.caption || ""} placeholder="caption"
